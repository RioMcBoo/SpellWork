...
namespace SpellWork
{
    public static class MySqlConnection
    {
        ...
        {
            if (WowData.SpellInfoStore.ContainsKey((int)id))
                return WowData.SpellInfoStore[(int)id].NameAndSubname;
                ...
                using (var command = new MySqlCommand(spellsQuery, conn))
                {
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            if (WowData.SpellDuration.TryGetValue(spellInfo.Misc.DurationIndex, out var duration))
                                spellInfo.DurationEntry = duration;

                            if (WowData.SpellRange.TryGetValue(spellInfo.Misc.RangeIndex, out var range))
                                spellInfo.Range = range;

                            WowData.SpellInfoStore[spellId] = spellInfo;
                        }
                    }
                }
                ...
                using (var command = new MySqlCommand(effectsQuery, conn))
                {
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            var spellId = (int)reader.GetUInt32(0);
                            if (!WowData.SpellInfoStore.TryGetValue((int)spellId, out var spellInfo))
                                continue;
